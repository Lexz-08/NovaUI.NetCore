...
namespace NovaUI.NetCore.Windows
{
    ...
    {
        private int v = 2;
        private Win32.MARGINS margins = new()
        {
            leftWidth = 0,
            rightWidth = 0,
            topHeight = 0,
        ...
        private System.Windows.Forms.Timer fadeTimer = new() { Interval = 10 };
		...
		public new FormWindowState WindowState
		{
			...
			set
			{
				...
				if (value != prevState) OnWindowStateChanged(new(prevState, value));
			}
		}
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			header = new(0, y, Width, headerHeight);
			...
			topLeft = new(0, 0, resizeWidth, resizeWidth);
			top = new(resizeWidth, 0, Width - (resizeWidth * 2), resizeWidth);
			topRight = new(Width - resizeWidth, 0, resizeWidth, resizeWidth);
			left = new(0, resizeWidth, resizeWidth, Height - (resizeWidth * 2));
			right = new(Width - resizeWidth, resizeWidth, resizeWidth, Height - (resizeWidth * 2));
			bottomLeft = new(0, Height - resizeWidth, resizeWidth, resizeWidth);
			bottom = new(resizeWidth, Height - resizeWidth, Width - (resizeWidth * 2), resizeWidth);
			bottomRight = new(Width - resizeWidth, Height - resizeWidth, resizeWidth, resizeWidth);

			Rectangle text = new(resizeWidth + x, resizeWidth + y - 1, Width - (resizeWidth * 2) - w, headerHeight - (resizeWidth * 2));
			if (ShowIcon)
			{
			...
			close = new(Width - headerHeight - resizeWidth - x, resizeWidth + y, headerHeight, headerHeight - (resizeWidth * 2));

			captionIcon = new(headerHeight - (resizeWidth * 2), headerHeight - (resizeWidth * 2));
			...
			if (debug)
			{
				...
				e.Graphics.FillRectangle(closeDebugBrush,
					new(close.X + ((close.Width - captionIcon.Width) / 2), close.Y, captionIcon.Width, captionIcon.Height));
			}
			...
			if (MinimizeBox)
			{
				minimize = new(close.X - headerHeight - x, resizeWidth + y, headerHeight, headerHeight - (resizeWidth * 2));

				if (MaximizeBox && canResize)
				{
					maximize = new(close.X - headerHeight - x, resizeWidth + y, headerHeight, headerHeight - (resizeWidth * 2));
					minimize = new(maximize.X - headerHeight - x, resizeWidth + y, headerHeight, headerHeight - (resizeWidth * 2));
		...
		protected override void OnLoad(EventArgs e)
		{
			...
			stateChangeSize = ClientSize;
			Size = new(ClientSize.Width - 16, ClientSize.Height - 39);
